...
vendredi = [24.0]*14 + [10.0]*1 + [24.0]*9
...
annuel_c = [t + 5.0 if t > 10 else 45.0 for t in annuel_h] # Adaptatif pour le cooling

# Sauvegarde en CSV (sans en-tête)
df = pd.DataFrame({'heating': annuel_h, 'cooling': annuel_c})
df.to_csv('dataset/modèle habitation/consignes_hvac.csv', index=False, header=False)
