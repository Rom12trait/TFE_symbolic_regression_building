...
for name, model in models.items():
    ...
    for day in selected_days:
        ...
        #pour une éqation pysr, faut l'intégrer manuellement dans l'optimiseur
        model_resolved, _ = optimizer.solve(prices, tout, t_init, year = yeartype, mode = "linear")

        # === APPEL DU POST-TRAITEMENT ===
        p_heat_opt, p_cool_opt, t_zone_opt, cout_instant_opt, total_cost = optimizer.post_process_hvac_simultaneity(
            model_resolved= model_resolved,
            prices_vector=prices,
            eta_h=optimizer.params["eta_h"],
            eta_c=optimizer.params["eta_c"],
            dt=optimizer.params["dt"]
        )
