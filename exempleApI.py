...
#building = BuildingModel("dataset/ModeleHabitation/model_annee_classique.idf","dataset/Meteo/Brussels.Natl.AP_BEL.epw",
 #                        "opti/bat")
from pyenergyplus.api import EnergyPlusAPI

class OptimizedBuildingControl:
    def __init__(self, results_opti):
        self.api = EnergyPlusAPI()
        self.results_opti = results_opti  # Ton dictionnaire results_all_days
        self.handles_initialized = False

        # Initialisation des handles d'actuators
        self.h_tset_heat = -1
        self.h_tset_cool = -1

    def callback_function(self, state):
        # 1. Attendre la fin du warmup pour ne pas fausser les données
        if self.api.exchange.warmup_flag(state):
            return

        # 2. Initialisation des handles au premier pas de temps réel
        if not self.handles_initialized:
            # Type: "Schedule:Constant" ou "Schedule:Compact", Control: "Schedule Value", Name: Nom dans l'IDF
            self.h_tset_heat = self.api.exchange.get_actuator_handle(state, "Schedule:Constant", "Schedule Value",
                                                                     "HTG_SETPOINT_SCHEDULE")
            self.h_tset_cool = self.api.exchange.get_actuator_handle(state, "Schedule:Constant", "Schedule Value",
                                                                     "CLG_SETPOINT_SCHEDULE")
            self.handles_initialized = True

        # 3. Récupérer le temps actuel (Heure + Minute)
        hour = self.api.exchange.hour(state)
        minute = self.api.exchange.minutes(state)
        day_of_year = self.api.exchange.day_of_year(state)

        # 4. Calcul de l'index (0 à 95)
        # 15 min = 4 pas par heure
        idx = int(hour * 4 + (minute / 15))
        if idx > 95: idx = 95

        # 5. Injection de ta température optimisée
        # On définit une consigne très proche autour de ta T_zone_opt pour "forcer" le système
        # Exemple pour un jour spécifique
        target_t = self.results_opti['2025-01-21']['T_zone'][idx]

        # On force le thermostat d'EnergyPlus à suivre cette cible exacte
        self.api.exchange.set_actuator_value(state, self.h_tset_heat, target_t - 0.1)
        self.api.exchange.set_actuator_value(state, self.h_tset_cool, target_t + 0.1)


# --- LANCEMENT DE LA SIMULATION ---
def run_validation(idf_path, weather_path, results_opti):
    api = EnergyPlusAPI()
    state = api.state_manager.new_state()

    control = OptimizedBuildingControl(results_opti)

    # On enregistre le callback avant que EnergyPlus ne commence à calculer le thermostat
    api.runtime.callback_begin_system_timestep_before_predictor(state, control.callback_function)

    # Lancement (similaire à une commande terminal)
    api.runtime.run_energyplus(state, ['-d', 'output', '-w', weather_path, idf_path])

run_validation("dataset/ModeleHabitation/anneeClassique/model_annee_classique.idf",
               "dataset/Meteo/Brussels.Natl.AP_BEL.epw", results_all_days)